...
I used Thymeleaf html template engine to generate fully functional html document and with Spring MVC supply necessary data model when a request come in.

JPA implemented by Hibernate is used to manage relational data. The tables in the databses are generated by the @Entity classes defined.

Spring data JPA is used in the repository layer to simplify JPA and benefit from its code-generation tool.

In the application.properties file, I added the necessary configuration to connect to the datasource.

I used mySQL database for the main project, and H2-database for the Selenium testing.

I changed the project starter code srtructure so it fits my needed changes concerning Entities and JPA-Hibernate.
